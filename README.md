...
Standard RAG fails in two compounding ways on heterogeneous corpora:

1. **Topic contamination at retrieval** — A query about Korean visa renewal pulls back trading documents that share surface vocabulary ("application," "filing," "deadline"). Flat dense search has no representation of subject domain — it only sees vector similarity.

2. **Generic generation** — Even with correct retrieval, a single general-purpose LLM produces hedged, undifferentiated answers. A finance question needs a model trained on finance reasoning. A legal question needs a model trained to refuse when the answer isn't in context — a behavior generic instruction-tuned models do poorly..

PRISM-RAG fixes both layers:
* **Retrieval:** documents are organized into a RAPTOR-style hierarchical topic tree (Sarthi et al., ICLR 2024). Queries walk down a tree of cluster summaries before touching leaf chunks — domain filtering is structural, not learned.
* **Generation:** three domain-specialist worker models (AI / Trading / Korean Immigration Law), each fine-tuned from a shared Qwen2.5 base and quantized to AWQ W4A16 for efficient serving.
...
PRISM-RAG:       "This query is about Korean immigration law"
                     ...
                     → retrieve only from the relevant cluster
                     → AWQ-quantized law worker generates the answer
                       (trained to refuse when answer not in context)
```

The tree is built offline (clustering + LLM-generated summaries). At query time, a LangGraph gateway routes to the matching subtree and dispatches the retrieved evidence to the corresponding domain worker.
